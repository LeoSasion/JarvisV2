...
// Standalone Explorer bridge ABI. This DLL owns lifecycle policy only. It
// contains no hook installer, process enumeration, remote-memory transport or
// Explorer mutation. A later exact-thread transport must be reviewed
// separately and linked into the same module before it can publish callbacks.

inline constexpr std::uint32_t JARVIS_EXPLORER_BRIDGE_CORE_ABI_VERSION = 2U;
...
static_assert(sizeof(jarvis_bridge_core_response) == 64U);
